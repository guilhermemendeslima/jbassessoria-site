          ...
          >
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6 p-8 bg-white/5 rounded-2xl backdrop-blur-sm transform-style-preserve-3d hover:transform-style-3d transition-all duration-500">
              {icons.map(({ Icon, delay }, index) => (
                <motion.div
                  key={index}
                  className="aspect-square bg-tertiary/10 rounded-xl flex items-center justify-center transform-style-preserve-3d hover:transform-style-3d transition-all duration-300"
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
